...
%videoFReader   = vision.VideoFileReader('C:\Users\Oluwole_Jnr\Desktop\University of Leeds\Main Project\Project_MATLab\Training\My Test Videos\Test Video 3 (Reduced).avi');
...
tic
[ROI sceneAvailable detectedRegion] = sceneClassifier(frame,0,1,0,0);
detectionTime = toc;
cnt=0;
%end
...
if sceneAvailable==1
    ...
    %Attempt to watermark video stream with detected image details
     ...
     framesPerSec = 1/(detectionTime+classificationTime);
     framesPerSecStr = num2str(framesPerSec);
     ctStr = strcat('CLASSF. TIME :- ',classificationTimeStr, ' second(s) ');
     dtStr = strcat('DETECTION TIME :- ',detectionTimeStr, ' second(s) ');
     fpsStr = strcat('FRAMS PER. SEC =  ',framesPerSecStr);
     
     textPos = [0 (yDim-30); 0 0; 0 30; 0 60];
     txt{1}=str;txt{2}=dtStr;txt{3}=ctStr;txt{4} = fpsStr;
